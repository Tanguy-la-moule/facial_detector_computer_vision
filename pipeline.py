...
def pass_frames_through_pipeline(name, frame_rate, frame_max, video_path='./video/', image_path='./img/'):
    frames = []
    frame_number = 0
    ...
    make_video_from_frames(frames, video_name, grey=True, video_path=video_path)
    ...
    for threshold in [20]: # 25
        ...
        make_video_from_frames(thresholded_frames, video_name, grey=True, video_path=video_path)
    ...
    for conv_size in [5]:  # 3
        ...
        make_video_from_frames(median_filtered_frames, video_name, grey=True, video_path=video_path)
    ...
    for closing_size in [20]:  # 10, 30
        ...
        make_video_from_frames(masks_results, video_name, grey=True, video_path=video_path)
        print('Done step 4.1 - masks operations ' + str(closing_size))

    # creating bounding boxes
    frame_number = 0
    bbs_groups_by_frame = []
    original_images = load_frames(name, frame_rate, frame_max, image_path='./img/', grayscale=False)
    for min_area in [500]:
    ...
    make_video_from_frames(boxed_masks, video_name, grey=False, video_path=video_path)
    ...
    make_video_from_frames(boxed_images, video_name, grey=False, video_path=video_path)
    ...
    for min_area in [20000]:
        ...
        save_pictures(pictures, name + '_7_area_' + str(min_area) + '_', image_path='./video_pipeline/')
    ...
    make_video_from_frames(modified_boxed_images, video_name, grey=False, video_path=video_path)
    ...
    for min_area in [20000]:
        ...
        pictures = get_pictures_from_bounding_boxes(best_bbs, original_images)
        save_pictures(pictures, name + '_9_area_' + str(min_area) + '_', image_path='./video_pipeline/')
    print('Done step 9 - extracting faces from corrected bounding boxes - saved ' + str(len(pictures)) + ' pictures')


# pass_frames_through_pipeline('ben', 1, 207, video_path='./video_pipeline/')
pass_frames_through_pipeline('tanguy', 1, 447, video_path='./video_pipeline/')
# create_frames_from_video('tanguy_8_absdiff_threshold_20_median_5_mask_20_contouring_100_color_modified', image_path='./test/', video_path='./video_pipeline/')
